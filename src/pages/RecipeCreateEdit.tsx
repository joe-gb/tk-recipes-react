...
import {BackButton, Button, CrossButton, NeutralButton} from "../components/Button"
...
function CreateEditForm() {
    ...
    const [createdRecipe, createLoading, createError, triggerSave] = RecipeService.SaveRecipe(recipe)
    const [getRecipe, getLoading, getError, TriggerGet]: [Recipe, boolean, AxiosError, Function] = RecipeService.GetRecipe(id, true)
    ...
    if ( getError ){
        return (
            ...
            <Text>There was an error retrieving this recipe: {getError.message}</Text>
        </>)
    }
    ...
    const onChangeIngredient = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const error = (event.target.value == '') ? "Ingredient name cannot be blank" : ""
        setValidationerrors({...validationerrors, ingredient: error})
    };
        ...
        if (recipe.ingredients) {
            const list = recipe.ingredients.map(
                (ingredient: Ingredient, index) =>
                    <ListItem key={index.toString()}>{ingredient.name}
                        {
                            showRemove == true &&
                            <CrossButton title="Delete ingredient" key={index + "button"} type="button"
                                         onClick={() => delete_ingredient(index)}>Remove</CrossButton>
                        }
                    </ListItem>
            )
            return list
    ...
    if (stage == 1) {
        return (
            ...
            <form onSubmit={onSubmitIngredient}>
                ...
                <Input label="What goes in your recipe?" placeholder={"Enter ingredient"} autoFocus name="ingredient"
                       value={ingredient}
                       error={validationerrors.ingredient}
                       onChange={onChangeIngredient} type="text"/>
                <Button type="submit">Add to recipe</Button>
                {ListIngredients(true)}
                <Row>
                    <NeutralButton type="button" onClick={back}>Back</NeutralButton>
                    <Button type="button" onClick={next}>Done</Button>
                </Row>
            ...
            <>
                {createError &&
                    <>
                        ...
                        <Text>There was an error creating this recipe: {createError.message}</Text>
                ...
                {(createdRecipe && !createError) &&
                    <>
                        ...
                        <Successbox>
                            <h3>Success</h3>
                            <p>Your recipe <strong>{createdRecipe.name}</strong> is saved with
                                reference {createdRecipe.id}</p>
                        </Successbox>
                        <Button onClick={startOver}>Start over</Button>
                    </>
                }
                ...
                <>
                    ...
                    <form onSubmit={onSubmitRecipeName}>
                        <Label>
                            ...
                            <Input error={validationerrors.name} placeholder="Recipe name" autoFocus name="name" value={recipe.name}
                                   onChange={onChangeRecipeName} type="text"/>
                        </Label>
